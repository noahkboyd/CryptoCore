 ...
 *   2. Use the key to generate the corresponding schedule (full, encryption focused, decryption focused)
...
// All schedules can do all actions but they are size optimized for different use cases
...
/* --- Encryption schedule types --- (excels at encryption, incurs cost for decryption) */
...
/* --- Decryption schedule types --- (excels at decryption, incurs cost for encryption)*/
...
INLINE void aes128_load_key_enc_only(const aes128_key_t* key, aes128_sched_enc_t* schedule);
INLINE void aes192_load_key_enc_only(const aes192_key_t* key, aes192_sched_enc_t* schedule);
INLINE void aes256_load_key_enc_only(const aes256_key_t* key, aes256_sched_enc_t* schedule);
